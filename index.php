...
foreach($resf->hits->hits as $hit){
    ?><img src="<?= $hit->_source->images[0]->urls->huge->url ?>" ><?php
//    var_dump($hit->_source->images[0]->urls->huge->url);
}
?>
    <pre><?php var_dump($work); ?></pre><?php
?>
    <pre><?php var_dump($resf->hits)?></pre>
<a href="<?= $work['image']?>">Link</a>